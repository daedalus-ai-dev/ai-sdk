...
describe('agent().prompt()', () => {
  ...
  it('handles tool not found gracefully', async () => {
    ...
    const response = assertComplete(
      await agent({ instructions: 'Test.', provider }).prompt('Do something'),
    );
  ...
  it('throws when no provider is configured', async () => {
    ...
    await expect(agent({ instructions: 'No provider.' }).prompt('Hi')).rejects.toThrow(
...
describe('checkpointing', () => {
  ...
  it('resumes from a checkpoint and continues the loop', async () => {
    ...
    const provider = mockProvider([
      {
        content: [
          { type: 'tool_use', id: 'tu_1', name: 'ask_user', input: { question: 'Your name?' } },
        ],
        stopReason: 'tool_use',
      },
      {
...
describe('runAgent()', () => {
  it('works with class-based agent', async () => {
    ...
    const response = assertComplete(
      await runAgent({ instructions: () => 'You are a test agent.' }, 'Hello', { provider }),
    );
